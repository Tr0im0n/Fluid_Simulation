...
def calc_things(positions: NDArray[np.float32], h: float, spl: SpatialPartitionList, rho_0: float):
    print("entered calc things")
    epsilon = 1e-9
    PI = np.pi
    h2 = h**2
    h5 = h**5
    ...
    center_poss = np.take(positions, center_indices_flat, axis=0)
    neighbor_poss = np.take(positions, neighbor_indices_flat, axis=0)
